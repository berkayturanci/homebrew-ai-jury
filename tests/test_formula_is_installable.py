...
* **Online** — download every url and hash it, exactly as `brew` does. Opt-in
  via ``AI_JURY_TAP_CHECK_EXTERNAL=1`` so the offline suite stays hermetic, and
  wired into CI where the value of looking is highest.

The plural is deliberate even though this formula currently vendors nothing: the
day it gains a `resource`, that resource's url and digest are a second thing
`brew` downloads and checksums, and the sync job verifies only the top-level
one. The sibling tap already has that case, and keel#787 is what a broken
vendored dependency looks like from a user's side — every command dying on an
...
FORMULA = Path(__file__).resolve().parent.parent / "Formula" / "ai-jury.rb"
ONLINE = os.environ.get("AI_JURY_TAP_CHECK_EXTERNAL") == "1"
...
class TheFormulaIsWellFormed(unittest.TestCase):
    ...
    def test_the_file_was_read(self):
        ...
        self.assertIn("class AiJury < Formula", self.text)
    ...
    def test_there_is_at_least_the_source_artifact(self):
    ...
    def test_a_version_named_in_the_test_block_matches_the_url(self):
        ...
        This formula's `test do` does pin one — `jury <version>` — so the
        agreement is live here and the assertion has teeth. The sibling tap's
        formula asserts only the binary's name, which is a legitimate choice; a
        test that demanded otherwise would be inventing a rule rather than
        guarding one, so the check skips instead of failing there.
        """
        url_version = re.search(r"ai_jury-(\d+\.\d+\.\d+)\.tar\.gz", self.text)
...
@unittest.skipUnless(ONLINE, "set AI_JURY_TAP_CHECK_EXTERNAL=1 to fetch the artifacts")
class EveryArtifactHashesToItsDeclaredDigest(unittest.TestCase):
    ...
    def test_the_tap_is_serving_the_current_release(self):
        ...
        url_version = re.search(
            r"ai_jury-(\d+\.\d+\.\d+)\.tar\.gz", FORMULA.read_text(encoding="utf-8")
        )
        self.assertIsNotNone(url_version, "no versioned source url")
        try:
            request = urllib.request.Request(
                "https://api.github.com/repos/berkayturanci/ai-jury/releases/latest",
    ...
    def test_every_url_resolves_and_hashes_to_its_digest(self):
        ...
        for url, declared in pairs():
            with self.subTest(url=url):
                ...
                checked += 1
        self.assertGreaterEqual(checked, 1, "nothing was actually downloaded")
